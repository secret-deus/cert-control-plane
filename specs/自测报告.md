...
| TC-E2E-002 数据库健康检查 | `GET /readyz` | 通过，返回 `{"status":"ok","db":"connected"}`；`/healthz` 仅返回 liveness |
...
| TC-SINGLE-004 健康检查 | `GET /healthz` + `GET /readyz` | 通过，`/healthz` 返回 `{"status":"ok"}`，`/readyz` 返回 `{"status":"ok","db":"connected"}` |
...
| 健康检查 | `curl -fsS http://localhost:8080/healthz` + `/readyz` | 通过，`/healthz` 为 liveness，`/readyz` 返回 DB connected |
...
### M4 生产 Compose Smoke 截图 (2026-05-11)

| 节点 | 证据 |
|------|------|
| Dashboard 登录后渲染 | ![M4 Compose Dashboard](screenshots/m4-compose-dashboard-2026-05-11.png) |
...
- `/readyz` 返回 `{"status":"ok","db":"connected"}`
- `/metrics` 包含 `certcp_up 1`、`certcp_db_up 1`
- 浏览器真实输入 Admin API Key 后进入 `/dashboard`
...
| Go Agent 下发 | `COMPOSE_PROJECT_NAME=certcp_go_smoke SMOKE_EDGE_OVERLAY=tools/agent-smoke/docker-compose.edge-go.yml SMOKE_PUBLIC_PORT=9445 bash tools/agent-smoke/run-live-smoke.sh` | 通过，边缘 nginx 写入并 reload；覆盖注册临时失败重试后的真实链路 |
...
| Go Agent 当前 serial | `37A568E00001EFA53240104083A864F25CD29706` |
...
| 健康检查 | 通过，`GET /healthz` 返回 `{"status":"ok"}`，`GET /readyz` 返回 DB connected |
...
## 2026-05-09 Kubernetes Secret V1 真实 minikube E2E

### 验证范围

本轮验证的是 Phase 5 V1 的真实 Kubernetes Secret 写入能力，不使用 mock Kubernetes API。

| 能力 | 验证结果 |
|------|----------|
| ServiceAccount kubeconfig 上传 | 通过 |
| 只读 Test Connection | 通过，`/version` 与默认 namespace GET 均成功 |
| 新 TLS Secret 创建 | 通过，`new-tls` 写入后 read-back serial 一致 |
| 已有 Secret adopt | 通过，`replace-tls` 被平台接管并写入 annotations |
| 新证书替换旧证书 | 通过，同 CN 新证书替换后 read-back serial 更新 |
| 最近一次回滚 | 通过，回滚到上一份 Secret snapshot |
| 手动改变部署目标 | 通过，同一证书部署到新目标 `moved-tls` |
| 手动 validate | 通过，三个 assignment 均产生 validate operation |
| 更新 kubeconfig 后自动 validate | 通过，真实 SQLite 集成测试覆盖 operation 生成与 health 状态更新 |
| 浏览器真实页面巡检 | 通过，页面显示 assignment、operation、dry-run diff |

### 执行命令

```bash
LOCAL_PORT=18081 bash scripts/minikube-deploy.sh
kubectl -n cert-control-plane port-forward svc/cert-control-plane 18081:8080
ADMIN_API_KEY="$(kubectl -n cert-control-plane get secret cert-control-plane-secrets -o jsonpath='{.data.ADMIN_API_KEY}' | base64 -d)" \
CONTROL_PLANE_URL=http://127.0.0.1:18081 \
...
```text
[1/8] Create cluster target
[2/8] Deploy new Secret
serial=50695FB1DE65533B86C30D6B07962771271CE0A2
[3/8] Adopt existing old Secret
[4/8] Replace old Secret with renewed cert
serial=4F4AD27CE6A3F3B9D3F35E1B8E2C8B57CB35D4FB
[5/8] Roll back latest replacement
serial=1EE5D6406A82714D1E18CE27FDD7BF3F15FA908B
[6/8] Manually change deployment target
serial=50695FB1DE65533B86C30D6B07962771271CE0A2
[7/8] Validate assignments
[8/8] Done
K8s Secret E2E completed against http://127.0.0.1:18081 namespace=cert-control-plane-e2e
```

### 浏览器截图

#### Kubernetes Secret assignments 与操作历史

![Kubernetes Secret assignments](screenshots/browser-20260509-k8s-secrets-overview.png)

#### Kubernetes Secret dry-run diff

![Kubernetes Secret dry-run diff](screenshots/browser-20260509-k8s-dry-run.png)

### 验证结论

Kubernetes Secret V1 已具备最小可用闭环：平台可通过 SA kubeconfig 连接业务集群，按显式 assignment 对单个 TLS Secret 做 create/adopt/update/rollback/validate，并在浏览器中查看 read-back 状态、serial、operation history 和 dry-run diff。当前实现仍保持 V1 边界：不管理 workload、不读取 Ingress/Gateway、不删除业务 Secret、不做批量 rollout。

## 2026-05-09 Kubernetes Secret 真实拓扑模拟

### 模拟拓扑

本轮不使用 mock。中心节点为 minikube 内已部署的 `cert-control-plane`，通过 `http://127.0.0.1:18081` 访问；三个目标节点用三个独立 namespace 和三份 ServiceAccount kubeconfig 模拟。

| 角色 | Kubernetes namespace | 凭证边界 | 说明 |
|------|----------------------|----------|------|
| 中心节点 | `cert-control-plane` | Admin API Key | 已部署的控制面服务与 PostgreSQL |
| Agent/目标 A | `certcp-real-edge-a` | namespace-scoped ServiceAccount | 新证书首次部署 |
| Agent/目标 B | `certcp-real-edge-b` | namespace-scoped ServiceAccount | adopt、替换、最近一次回滚 |
| Agent/目标 C | `certcp-real-edge-c` | namespace-scoped ServiceAccount | 手动变更部署机器 |
...
| 验证项 | 结果 |
|--------|------|
| 三个目标 namespace 与 SA kubeconfig | 通过 |
| 三个 Kubernetes target 注册 | 通过，均通过 test connection |
| 新证书部署到目标 A | 通过，K8s read-back serial 与上传证书一致 |
| 目标 B adopt 已存在 Secret | 通过 |
| 目标 B 新证书替换旧证书 | 通过，替换后 serial 更新 |
| 目标 B 最近一次回滚 | 通过，回滚后 serial 回到旧证书 |
| 手动改变部署机器到目标 C | 通过，同一证书部署到 C，并删除 A 的 assignment 记录 |
| 删除 assignment 后不删除业务 Secret | 通过，A 的 Secret 仍存在 |
| RBAC 负例 | 通过，A 的 SA kubeconfig 写 B namespace 被 `403` 拦截 |
| 浏览器真实巡检 | 通过，Playwright Chromium 登录页面并截图 |

### 关键输出

```text
run_id=20260509133949
control_plane=http://127.0.0.1:18081
edge_a_cluster_id=c139eff2-0040-4d46-bb1e-da5ff60a0dbd
edge_b_cluster_id=355a5b9e-ff5b-435b-b573-3d1545480a4b
edge_c_cluster_id=f141bde2-3e0e-464b-8153-1e1bd2601bd3
edge_a_assignment_deleted=d225575c-a252-4b6c-b4f7-dfabe0a84270
edge_b_assignment_active=86223358-8593-4b3f-b1bc-8ad316313f03
edge_c_assignment_active=2423f3a4-aa36-4293-8707-d2cae5a7c389
edge_a_expected_serial=40472FDF0407DE716E0CB1BE079F32243ED60E2F
edge_a_actual_serial=40472FDF0407DE716E0CB1BE079F32243ED60E2F
edge_a_secret_after_move_serial=40472FDF0407DE716E0CB1BE079F32243ED60E2F
edge_b_old_serial=0DC0ACFF375A6D2DF8549F17F3DACBA46FED34E2
edge_b_renewed_expected_serial=77E2D997BBFBD54AABE7BCE53F572645D21FF2F4
edge_b_renewed_actual_serial=77E2D997BBFBD54AABE7BCE53F572645D21FF2F4
edge_b_rollback_serial=0DC0ACFF375A6D2DF8549F17F3DACBA46FED34E2
edge_c_serial=40472FDF0407DE716E0CB1BE079F32243ED60E2F
rbac_cross_namespace_status=403
edge_a_secret_subject_after_move=subject=CN=real-edge-a-20260509133949.example.test
edge_b_secret_subject_after_rollback=subject=CN=real-edge-b-20260509133949.example.test
edge_c_secret_subject=subject=CN=real-edge-a-20260509133949.example.test
```

### Kubernetes read-back

```text
certcp-real-edge-a/api-tls  kubernetes.io/tls  external-cert=df2c132a-116e-44f5-bcc9-973b5b5c5793
certcp-real-edge-b/api-tls  kubernetes.io/tls  external-cert=66a72f19-733c-4f3e-adfb-616aa7a75a6d
certcp-real-edge-c/api-tls  kubernetes.io/tls  external-cert=df2c132a-116e-44f5-bcc9-973b5b5c5793
```

### 浏览器截图

#### 三目标节点 Secret assignments 与操作历史

![Kubernetes Secret real simulation overview](screenshots/browser-20260509-real-simulation-overview.png)

#### 目标 B 回滚后的 dry-run 替换 diff

![Kubernetes Secret real simulation dry-run](screenshots/browser-20260509-real-simulation-dry-run.png)

### 验证结论

这轮测试覆盖了更接近生产的控制面到多目标节点链路：中心节点真实部署，三个目标节点使用独立 SA kubeconfig，证书写入和回读都经过 Kubernetes API。新增、替换、回滚、迁移部署机器、RBAC 越权拦截均已验证。当前 V1 仍按既定边界处理：删除 assignment 不删除业务 Secret；回滚后如果证书资产仍指向 renewed 版本，页面会显示 serial mismatch，并允许再次 dry-run/confirm 更新。
