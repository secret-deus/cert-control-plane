      ...
      <div className="flex flex-col gap-4 xl:flex-row xl:items-center xl:justify-between">
        <div className="flex items-start gap-3">
          <button
            type="button"
            onClick={onOpenMenu}
            className="mt-0.5 inline-flex h-10 w-10 items-center justify-center rounded-xl border border-white/10 bg-white/[0.03] text-slate-200 lg:hidden"
          >
            <Menu size={18} />
          </button>
          <div>
            <div className="text-[11px] font-medium tracking-[0.16em] text-neutral-500">Pages / {subtitle}</div>
            <h1 className="mt-2 text-3xl font-semibold tracking-tight text-white lg:text-[2.4rem]">{title}</h1>
          </div>
        </div>

        <div className="flex items-center gap-3 xl:justify-end">
          <button className="inline-flex items-center gap-2 rounded-2xl border border-white/8 bg-white/[0.03] px-3 py-2 text-xs font-medium text-neutral-200 transition-colors hover:border-white/12 hover:text-white">
  ...
  const sidebarContent = (
    <>
      <div className="border-b border-white/6 px-4 py-5">
        <div className="flex items-center gap-3 px-2 py-2">
          ...
          <div>
            ...
            <div className="mt-1 text-xs tracking-[0.16em] text-neutral-500">CONTROL PLANE</div>
          </div>
        </div>
      </div>
              ...
              className={() =>
                `flex items-center gap-3 rounded-[18px] border px-4 py-4 transition-colors ${
                  isActive
                    ...
                    : 'border-transparent text-neutral-400 hover:border-transparent hover:bg-white/[0.02] hover:text-white'
                }`
              }
            >
              <div className={`rounded-[14px] p-2.5 ${isActive ? 'bg-white/[0.05] text-white' : 'bg-transparent text-neutral-500'}`}>
      ...
      <div className="px-3 pb-4">
        <div className="rounded-[18px] border border-white/8 bg-white/[0.02] px-4 py-4 text-xs text-slate-400">
        ...
        <button
          onClick={onLogout}
          className="mt-3 w-full rounded-[18px] border border-white/8 px-4 py-3 text-left text-sm text-neutral-400 transition-colors hover:border-white/12 hover:bg-white/[0.03] hover:text-white"
        >
          退出登录
        </button>
      </div>
          ...
          >
            <div className="flex items-center justify-end px-4 pt-4">
              <button
                type="button"
                onClick={() => setMobileMenuOpen(false)}
                  className="inline-flex h-10 w-10 items-center justify-center rounded-[18px] border border-white/8 bg-white/[0.03] text-slate-200"
              >
                <X size={18} />
              </button>
            </div>
